# -*- coding: utf-8 -*-
"""Add CDN gallery to shiftCamp.html from images/shiftCampImages.html."""
...
page_path = ROOT / "shiftCamp.html"
pg_path = ROOT / "pg.html"

urls = [
    u.strip()
    for u in (ROOT / "images" / "shiftCampImages.html").read_text(encoding="utf-8").splitlines()
...
if 'id="galleryRoot"' in page:
    raise SystemExit("shiftCamp.html already has a gallery")
...
gallery_js_items = ",\n        ".join(
    f'{{ src: "{u}", alt: "Shift Camp photo {i}" }}' for i, u in enumerate(urls, 1)
)
gallery_config = f"""      /* ============================================================
    ...
    <section class="program-gallery" id="program-gallery">
      <div class="container">
        <div class="section-header">
          ...
          <h2 class="section-heading anim-item">Inside the Shift Camp Experience</h2>
          <p class="section-intro anim-item">Explore highlights from previous Shift Camp groups.</p>
        </div>
        <div class="gallery-slider" id="galleryRoot" aria-roledescription="carousel" aria-label="Shift Camp photo gallery"></div>
      </div>
    </section>
...
print(f"shiftCamp.html: added gallery with {len(urls)} CDN urls")
