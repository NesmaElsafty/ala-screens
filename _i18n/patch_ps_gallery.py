...
urls = [
    u.strip()
    for u in (ROOT / "images" / "psImages.html").read_text(encoding="utf-8").splitlines()
    if u.strip().startswith("http")
]
path = ROOT / "ps.html"
html = path.read_text(encoding="utf-8")

# Detect existing alt label pattern
m = re.search(
    r'alt:\s*"([^"]*program photo )"\s*\+\s*\(index \+ 1\)',
    html,
)
alt_prefix = m.group(1) if m else "Public Speaking program photo "
...
print(f"ps.html gallery -> {len(urls)} CDN urls (alt prefix: {alt_prefix!r})")
