# -*- coding: utf-8 -*-
"""Add CDN gallery to silentCamp.html from images/silentCampImages.html."""
...
page_path = ROOT / "silentCamp.html"
pg_path = ROOT / "pg.html"

urls = [
    u.strip()
    for u in (ROOT / "images" / "silentCampImages.html").read_text(encoding="utf-8").splitlines()
...
if 'id="galleryRoot"' in page:
    raise SystemExit("silentCamp.html already has a gallery")
...
gallery_js_items = ",\n        ".join(
    f'{{ src: "{u}", alt: "Silent Camp photo {i}" }}' for i, u in enumerate(urls, 1)
)
gallery_config = f"""      /* ============================================================
...
if "body.lightbox-open" not in page:
    page = page.replace(
        """    body {
...
if "/* Gallery slider */" not in page:
    if "/* Final CTA */" not in page:
        raise SystemExit("Final CTA marker missing")
    page = page.replace("    /* Final CTA */", gallery_css + "    /* Final CTA */", 1)
    ...
    <section class="program-gallery" id="program-gallery">
      <div class="container">
        <div class="section-header">
          ...
          <h2 class="section-heading anim-item">Inside the Silent Camp Experience</h2>
          <p class="section-intro anim-item">Explore highlights from previous Silent Camp groups.</p>
        </div>
        <div class="gallery-slider" id="galleryRoot" aria-roledescription="carousel" aria-label="Silent Camp photo gallery"></div>
      </div>
    </section>
...
print(f"silentCamp.html: added gallery with {len(urls)} CDN urls")
